...
def seed_database():
    with app.app_context():
        ...
        c1 = Customer(
            name="John Doe",
            email="john@acmetractors.com",
            phone="555-0101",
            company_name="Acme Tractors",
        )
        c2 = Customer(
            name="Jane Smith",
            email="jane@globalauto.com",
            phone="555-0202",
            company_name="Global AutoParts",
        )
        ...
        s1 = SpringMaster(
            part_number="COMP-101",
            wire_diameter=2.5,
            material_spec="Stainless Steel 304",
        )
        s2 = SpringMaster(
            part_number="EXT-202", wire_diameter=1.2, material_spec="High Carbon Steel"
        )
        ...
        mat1 = Material(
            name="SS-304 Wire", specification="2.5mm roll", stock_quantity=500.0
        )
        mat2 = Material(
            name="HC Steel Wire", specification="1.2mm roll", stock_quantity=150.0
        )  # Low stock
        ...
        quote1 = Quotation(
            ...
            version_number=1,
            price=2500.00,
            est_delivery=dt.datetime.now(dt.timezone.utc).date() + dt.timedelta(days=5),
            is_accepted=True,
        )
        ...
        task1 = ProductionTask(
            ...
            status="Completed",
            scheduled_at=dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=8),
            completed_at=dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=7),
        )
        task2 = ProductionTask(
            ...
            material_id=mat1.material_id,
            status="Completed",
            scheduled_at=dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=6),
            completed_at=dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=5),
        )
        ...
        qr1 = QualityReport(
            ...
            inspector="Mike Foreman",
            result="Approved",
            report_date=dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=4),
        )
        db.session.add(qr1)
        db.session.commit()
        ...
        inv1 = Invoice(
            order_id=order1.order_id,
            amount=2500.00,
            issued_date=dt.datetime.now(dt.timezone.utc).date() - dt.timedelta(days=3),
            paid=True,
        )
        ...
        inv2 = Invoice(
            order_id=order1.order_id,
            amount=1200.50,
            issued_date=dt.datetime.now(dt.timezone.utc).date(),
            paid=False,
        )
